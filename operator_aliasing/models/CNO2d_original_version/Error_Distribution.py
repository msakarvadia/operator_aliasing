...
def load_data(
    ...
    if which_model == 'CNO':
        ...
        from Problems.CNOBenchmarks import AllenCahn
        ...
        from Problems.FNOBenchmarks import Airfoil
        from Problems.FNOBenchmarks import AllenCahn
        from Problems.FNOBenchmarks import ContTranslation
        from Problems.FNOBenchmarks import Darcy
        from Problems.FNOBenchmarks import DiscContTranslation
    ...
    if which_model == 'CNO':
        if which_example == 'shear_layer':
            ...
            example = SinFrequency(
                model_architecture_, device, batch_size, in_dist=in_dist
            )
        ...
        if which_example == 'shear_layer':
            ...
            example = SinFrequency(
                model_architecture_, device, batch_size, s=64, in_dist=in_dist
            )
        elif which_example == 'wave_0_5':
            example = WaveEquation(
                model_architecture_, device, batch_size, in_dist=in_dist
...
def error_distribution(
    which_model, model, testing_loader, p, N, device, which='shear_layer'
):
    E_diss = np.zeros(N)
    cnt = 0
    ...
    with torch.no_grad():
        for i, (inputs, outputs) in enumerate(testing_loader):
            ...
            if which_model == 'CNO':
                ...
                err = (
                    torch.mean(
                        abs(prediction[:, 0, :, :] - outputs[:, 0, :, :]) ** p,
                        (-2, -1),
                    )
                    / torch.mean(abs(outputs[:, 0, :, :]) ** p, (-2, -1))
                ) ** (1 / p) * 100
...
def plot_samples(
    which_model, data_loader, model, p, n, cmap='jet', which='shear_layer'
):
    cmap = 'gist_ncar'
...
def plot_samples_different_models(
    which_models, data_loaders, models, p, n, which='shear_layer'
):
    preds = []
    ...
    im1 = axes[0].imshow(
        inp, cmap=cmap, extent=(0, 1, 1, 0), interpolation='bicubic'
    )
    axes[0].invert_yaxis()
    ...
    im2 = axes[1].imshow(
        out, cmap=cmap, extent=(0, 1, 1, 0), interpolation='bicubic'
    )
    ...
    for m in range(M):
        ...
        im3 = axes[m + 2].imshow(
            preds[m], cmap=cmap, extent=(0, 1, 1, 0), interpolation='bicubic'
        )
...
if dist:
    modelFNO = torch.load(
        folder_FNO + '/model.pkl', map_location=torch.device(device)
    )
    modelCNO = torch.load(
        folder_CNO + '/model.pkl', map_location=torch.device(device)
    )

    modelFNO.device = device
    modelCNO.device = device

    data_loader_FNO = load_data(
        folder_FNO, 'FNO', device, which, in_dist=in_dist
    )
    data_loader_CNO = load_data(
        folder_CNO, 'CNO', device, which, in_dist=in_dist
    )

    data_loader_FNO.num_workers = 16
    data_loader_CNO.num_workers = 16

    E_FNO = error_distribution(
        which_model='FNO',
        model=modelFNO,
        testing_loader=data_loader_FNO,
        p=1,
        N=N,
        device=device,
        which=which,
    )
    E_CNO = error_distribution(
        which_model='CNO',
        model=modelCNO,
        testing_loader=data_loader_CNO,
        p=1,
        N=N,
        device=device,
        which=which,
    ...
    if write_spectra:
        ...
        avg, avg_inp, avg_out = avg_spectra(
            'CNO', modelCNO, data_loader_CNO, device, in_size=size
        )
        write_in_file_spectra(folder_CNO, spectra_name, avg, avg_inp, avg_out)

        avg, avg_inp, avg_out = avg_spectra(
            'FNO', modelFNO, data_loader_FNO, device, in_size=size
        )
    ...
    data_loader_FNO = load_data(
        folder_FNO, 'FNO', device, which, batch_size=1, in_dist=in_dist
    )
    data_loader_CNO = load_data(
        folder_CNO, 'CNO', device, which, batch_size=1, in_dist=in_dist
    )
