...
from utilities_nemo.data_loader import load_bathymetry
from utilities_nemo.data_loader import load_input_output
from utilities_nemo.train_util import normalize_bath_encode
from utilities_nemo.train_util import normalize_encode
...
class DarcyFlow:
    def __init__(
        self, network_properties, device, batch_size, training_samples
    ):
        ...
        training_inputs, training_outputs = (
            inputs[:training_samples],
            outputs[:training_samples],
        )
        testing_inputs, testing_outputs = inputs[1000:], outputs[1000:]

        self.model = ContConv2D(
            in_channels=1 + 2 * self.N_Fourier_F,  # Number of input channels.
        ...
        self.train_loader = DataLoader(
            TensorDataset(training_inputs, training_outputs),
            batch_size=batch_size,
            shuffle=True,
        )
        self.test_loader = DataLoader(
            TensorDataset(testing_inputs, testing_outputs),
            batch_size=batch_size,
            shuffle=False,
        )
...
class NavierStokes_VIDON:
    def __init__(
        ...
        training_inputs, training_outputs = self.get_data(
            'data/data_32_32__Navier_Stokes/training_data_grid_time_5.hdf5',
            1000,
        )
    ...
    def get_data(self, file, n_samples):
        ...
        with h5py.File(file, 'r') as f:
            ...
            return torch.tensor(input_data).type(torch.float32).permute(
                0, 3, 1, 2
            ), torch.tensor(output_data).type(torch.float32).permute(
                0, 3, 1, 2
            )
...
class ShearLayer64:
    def __init__(
        ...
        training_inputs, training_outputs = self.get_data(
            '/cluster/scratch/braonic/Euler_Shear/ddsl_N64/', 1024
        )
        ...
        for i in range(n_samples):
            ...
            if self.in_size < 64:
                input_data[i, 0] = downsample(
                    np.array(f.variables['u'][:]).reshape(1, 1, 64, 64),
                    self.in_size,
                )
            else:
...
class ShearLayer:
    def __init__(
        ...
        if cluster:
            training_inputs, training_outputs = self.get_data(
                '/cluster/scratch/braonic/Euler_Shear/' + file, 1024, cluster
            )
        else:
            training_inputs, training_outputs = self.get_data(
    ...
    def get_data(self, folder, n_samples, cluster):
        ...
        for i in range(n_samples):
            ...
            if self.in_size < 128:
                input_data[i, 0] = downsample(
                    np.array(f.variables['u'][:]).reshape(1, 1, 128, 128),
                    self.in_size,
                )
            else:
                input_data[i, 0] = np.array(f.variables['u'][:])
...
class ShearLayerRecursive:
    def __init__(
        ...
        training_inputs, training_outputs, self.rec_data = self.get_data(
            'data/Euler_Shear/ddsl_N128/', training_samples
        )
        ...
        testing_inputs = training_inputs[
            5 * training_samples : 5 * (training_samples + 64)
        ]
        testing_outputs = training_outputs[
            5 * training_samples : 5 * (training_samples + 64)
        ]
    ...
    def get_data(self, folder, n_samples):
        input_data = np.zeros(
            (
                5 * (n_samples + 64),
                1 + 2 * self.N_Fourier_F,
                self.in_size,
                self.in_size,
            )
        )
        output_data = np.zeros(
            (5 * (n_samples + 64), 1, self.in_size, self.in_size)
        )
        ...
        for i in c:
            for t in range(0, 9, 2):
                file_input = (
                    folder + 'sample_' + str(i) + '_time_' + str(t) + '.nc'
                )
                file_output = (
                    folder + 'sample_' + str(i) + '_time_' + str(t + 2) + '.nc'
                )

                f = netCDF4.Dataset(file_input, 'r')
                if self.in_size < 128:
                    input_data[cnt, 0] = downsample(
                        np.array(f.variables['u'][:]).reshape(1, 1, 128, 128),
                        self.in_size,
                    )
                else:
        ...
        return (
            torch.tensor(input_data).type(torch.float32),
            torch.tensor(output_data).type(torch.float32),
            torch.tensor(rec_data).type(torch.float32),
        )
...
class ShearLayerRecursiveOut:
    ...
    def get_data(self, folder, n_samples):
        ...
        output_data = np.zeros(
            (2 * n_samples + 5 * 64, 1, self.in_size, self.in_size)
        )
...
class Translation:
    def __init__(self, network_properties, device, batch_size):
        ...
        file_train = (
            'data/RotationTranslation/TrainingData_RotationTranslation.h5'
        )
        file_test = 'data/RotationTranslation/TestData_RotationTranslation.h5'

        training_inputs, training_outputs = self.get_data(
            file_train, 500, device
        )
    ...
    def get_data(self, file, n_samples, device):
        ...
        input_data = (
            torch.tensor(
                np.zeros((n_samples, 1 + 2 * self.N_Fourier_F, 64, 64))
            )
            .type(torch.float32)
            .to(device)
        )
        output_data = (
            torch.tensor(np.zeros((n_samples, 1, 64, 64)))
            .type(torch.float32)
            .to(device)
        )
        with h5py.File(file, 'r') as f:
            ...
            for i in range(n_samples):
                if one:
                    ...
                    name = 'sample_' + str(i)
                input_dist = (
                    torch.tensor(f[name]['initial_dist'][:].reshape(64, 64))
                    .type(torch.float32)
                    .to(device)
                )
                output_dist = (
                    torch.tensor(f[name]['final_dist'][:].reshape(64, 64))
                    .type(torch.float32)
                    .to(device)
                )
    ...
    def get_grid(self):
        with h5py.File(
            'data/RotationTranslation/TrainingData_RotationTranslation.h5', 'r'
        ) as f:
            grid = (
                torch.tensor(f['grid'][:]).type(torch.float32).permute(2, 0, 1)
            ).numpy()
        return grid
...
class Rotation:
    ...
    def get_grid(self):
        with h5py.File(
            'data/RotationTranslation/TrainingData_Rotation.h5', 'r'
        ) as f:
            grid = (
                torch.tensor(f['grid'][:]).type(torch.float32).permute(2, 0, 1)
            ).numpy()
...
class TranslationRotation:
    ...
    def get_grid(self):
        with h5py.File(
            'data/RotationTranslation/TestData_RotationEasier.h5', 'r'
        ) as f:
            grid = (
                torch.tensor(f['grid'][:]).type(torch.float32).permute(2, 0, 1)
            ).numpy()
...
class NavierStokes:
    def __init__(self, network_properties, device, batch_size):
        training_inputs, training_outputs = self.get_data(
            'data/data_32_32__Navier_Stokes/training_data.hdf5', 500
        )
        testing_inputs, testing_outputs = self.get_data(
            'data/data_32_32__Navier_Stokes/validation_data.hdf5', 100
        )
    ...
    def get_data(self, file, n_samples):
        ...
        with h5py.File(file, 'r') as f:
            ...
            for i in range(n_samples):
                ...
                input_data[i] = torch.tensor(
                    np.array(f[a_group_key[0]][s_in])
                ).reshape(1, 32, 32)
                output_data[i] = torch.tensor(
                    np.array(f[a_group_key[1]][s_out]).reshape(-1, 32, 32)[-1]
                ).reshape(1, 32, 32)
    ...
    def get_grid(self):
        with h5py.File(
            'data/data_32_32__Navier_Stokes/training_data.hdf5', 'r'
        ) as f:
            a_group_key = list(f.keys())
...
class NEMO:
    def __init__(
        ...
        train_a, train_u, test_a, test_u = self.get_data(
            ['ssh', 'bath', 'mslp'], 12
        )
        ...
        self.model = ContConv2D(
            in_channels=self.arg__size
            + 2 * self.N_Fourier_F,  # Number of input channels.
            in_size=in_size,  # Input spatial size: int or [width, height].
            cutoff_den=cutoff_den,
            N_layers=N_layers,
            N_res=N_res,
        ...
        self.train_loader = DataLoader(
            TensorDataset(train_a, train_u),
            batch_size=batch_size,
            shuffle=True,
        )
        self.test_loader = DataLoader(
            TensorDataset(test_a, test_u), batch_size=batch_size, shuffle=False
        )
    ...
    def get_data(self, arguments, n_months):
        ...
        for i in range(len(arguments)):
            ...
            if v != 'bath':
                ...
                if v == 'ssh':
                    inputs_v, outputs_v = load_input_output(
                        file=file_ssh,
                        n_months=n_months,
                        dL=dL,
                        Len1=Len1,
                    ...
                    inputs_test_v, outputs_test_v = load_input_output(
                        file=file_ssh_test,
                        n_months=n_months,
                        dL=dL,
                        Len1=Len1,
                ...
                inputs_V, outputs_V, extrema = normalize_encode(
                    inputs_V, outputs_V
                )
                ...
                bath = bath.reshape(
                    bath.shape[0], bath.shape[1], bath.shape[2], 1
                )
        ...
        input_data = np.zeros(
            (
                n_samples,
                self.arg__size + 2 * self.N_Fourier_F,
                self.in_size,
                self.in_size,
            )
        )
        ...
        return (
            torch.tensor(input_data[:ntrain]).type(torch.float32),
            torch.tensor(output_data[:ntrain]).type(torch.float32),
            torch.tensor(input_data[ntrain:]).type(torch.float32),
            torch.tensor(output_data[ntrain:]).type(torch.float32),
        )
...
class Advection:
    def __init__(self, network_properties, device, batch_size):
        ...
        self.train_loader = DataLoader(
            AdvectionDataset('training', self.N_Fourier_F),
            batch_size=batch_size,
            shuffle=True,
            num_workers=16,
        )
        self.test_loader = DataLoader(
            AdvectionDataset('testing', self.N_Fourier_F),
            batch_size=batch_size,
            shuffle=False,
            num_workers=16,
        )
