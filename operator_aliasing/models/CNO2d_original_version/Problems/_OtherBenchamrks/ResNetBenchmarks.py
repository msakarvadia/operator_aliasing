...
class ShearLayerDataset(Dataset):
    def __init__(
        ...
        if in_dist:
            ...
            if self.s == 64:
                ...
                self.folder = (
                    'data/Euler_Shear/ddsl_N128/'  # In-distribution file
                )
                self.N_max = 1024
        else:
            self.folder = (
                'data/Euler_Shear_0_05/ddsl_N128/'  # Out-of_-distribution file
            )
            self.N_max = 1024
    ...
    def __getitem__(self, index):
        if self.s == 64:
            inp = self.reader['Sample_' + str(index + self.start)]['input'][
                :
            ].reshape(self.s, self.s)
            ...
            if self.s < 128:
                ...
                inputs = np.array(f.variables['u'][:]).reshape(
                    1, self.s, self.s
                )
            f.close()
            ...
            if self.s < 128:
                ...
                labels = np.array(f.variables['u'][:]).reshape(
                    1, self.s, self.s
                )
...
class ShearLayer:
    def __init__(
        ...
        self.train_loader = DataLoader(
            ShearLayerDataset('training', 0, training_samples, s),
            batch_size=batch_size,
            shuffle=True,
            num_workers=8,
        )
        self.val_loader = DataLoader(
            ShearLayerDataset('validation', 0, training_samples, s),
            batch_size=batch_size,
            shuffle=False,
            num_workers=8,
        )
        ...
        self.test_loader = DataLoader(
            ShearLayerDataset('test', 0, training_samples, s, in_dist),
            batch_size=batch_size,
            shuffle=False,
