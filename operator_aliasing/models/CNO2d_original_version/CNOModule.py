...
from operator_aliasing.models.CNO2d_original_version.debug_tools import (
    format_tensor_size,
)
from operator_aliasing.models.CNO2d_original_version.Error_Distribution_VaryingResolution import (
    resize,
)

# from debug_tools import *
from operator_aliasing.models.CNO2d_original_version.training.filtered_networks import (  # Either "filtered LReLU" or regular LReLu
    LReLu,
)
from operator_aliasing.models.CNO2d_original_version.training.filtered_networks import (  # Either "filtered LReLU" or regular LReLu
    LReLu_regular,
...
class CNOBlock(nn.Module):
    def __init__(
        ...
        # ---------- Filter properties -----------
        self.citically_sampled = (
            False  # We use w_c = s/2.0001 --> NOT critically sampled
        )
        ...
        self.in_halfwidth = (
            half_width_mult * self.in_size - self.in_size / cutoff_den
        )
        self.out_halfwidth = (
            half_width_mult * self.out_size - self.out_size / cutoff_den
        )
        ...
        if activation == 'cno_lrelu':
            self.activation = LReLu(
                in_channels=self.in_channels,  # In _channels is not used in these settings
                out_channels=self.out_channels,
            ...
            self.activation = LReLu_torch(
                in_channels=self.out_channels,  # In _channels is not used in these settings
                out_channels=self.out_channels,
                in_size=self.in_size,
                out_size=self.out_size,
                in_sampling_rate=self.in_size,
...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        if activation == 'cno_lrelu':
            self.activation = LReLu(
                in_channels=self.channels,  # In _channels is not used in these settings
                out_channels=self.channels,
                in_size=self.size,
                out_size=self.size,
...
class CNO(nn.Module):
    def __init__(
        ...
        for i in range(1, self.N_layers):
            ...
            )  # Pad the outputs of the resnets
        ...
        for i in range(self.N_layers + 1):
            ...
            self.decoder_sizes.append(
                latent_size_out // 2 ** (self.N_layers - i)
            )
    ...
    def forward(self, x):
        # if inference dim different from train dim, project first
        original_size = x.shape[-1]
        x = resize(x, (self.in_size, self.in_size))
        ...
        # if inference dim different from train dim, project back
        x = resize(x, (original_size, original_size))
