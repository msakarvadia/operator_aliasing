...
def load_data(
    ...
    if which_model == 'CNO' or which_model == 'FNO':
        if which_example == 'shear_layer':
            model_architecture_['in_size'] = s
            example = ShearLayer(
                model_architecture_, device, batch_size, training_samples
            )
        else:
            raise ValueError()
    else:
...
def samples_ifft(u_hat):
    return scipy.fft.ifftn(
        u_hat, s=u_hat.shape[2:], norm='forward', workers=-1
    ).real
...
def downsample(u, N, fourier=False):
    if np.isrealobj(u):
        ...
        raise TypeError(
            f'Expected either real or complex valued array. Got {u.dtype}.'
        )
    d = u_hat.ndim - 2
    u_hat_down = None
    if d == 2:
        u_hat_down = np.zeros(
            (u_hat.shape[0], u_hat.shape[1], N, N), dtype=u_hat.dtype
        )
        u_hat_down[:, :, : N // 2 + 1, : N // 2 + 1] = u_hat[
            :, :, : N // 2 + 1, : N // 2 + 1
        ]
        u_hat_down[:, :, : N // 2 + 1, -N // 2 :] = u_hat[
            :, :, : N // 2 + 1, -N // 2 :
        ]
        u_hat_down[:, :, -N // 2 :, : N // 2 + 1] = u_hat[
            :, :, -N // 2 :, : N // 2 + 1
        ]
        u_hat_down[:, :, -N // 2 :, -N // 2 :] = u_hat[
            :, :, -N // 2 :, -N // 2 :
        ]
    else:
...
def plot_different_resolution(D, YTicks, title, loc_legend='upper right'):
    ...
    for k in D:
        ...
        axes.plot(
            R, E, '-o', zorder=3, label=k, color=color, linewidth=linewidth
        )
...
if not plot:
    ...
    for s_test in range(32, 129, 8):
        data_loader_CNO = load_data(
            folder_CNO, 'CNO', device, which, 64, batch, 1, s_test
        )
