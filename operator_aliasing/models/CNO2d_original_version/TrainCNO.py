...
df.to_csv(
    folder + '/training_properties.txt', header=False, index=True, mode='w'
)
...
if which_example == 'shear_layer':
    example = ShearLayer(
        model_architecture_, device, batch_size, training_samples, size=64
    )
...
best_model_testing_error = (
    1000  # Save the model once it has less than 1000% relative L1 error
)
...
for epoch in range(epochs):
    with tqdm(unit='batch', disable=False) as tepoch:
        ...
        for step, (input_batch, output_batch) in enumerate(train_loader):
            ...
            loss_f.backward()
            optimizer.step()
            train_mse = train_mse * step / (step + 1) + loss_f.item() / (
                step + 1
            )
            tepoch.set_postfix(
        ...
        with torch.no_grad():
            ...
            writer.add_scalar(
                'train_loss/train_loss_rel', train_relative_l2, epoch
            )
        ...
        tepoch.set_postfix(
            {
                ...
                'Relative Val loss': test_relative_l2,
            }
        )
        tepoch.close()
