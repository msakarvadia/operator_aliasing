...
from operator_aliasing.models.CNO2d_original_version.torch_utils import misc
from operator_aliasing.models.CNO2d_original_version.torch_utils import (
    persistence,
)
from operator_aliasing.models.CNO2d_original_version.torch_utils.ops import (
    filtered_lrelu,
)
...
class RadialConv2d(nn.Module):
    ...
    def forward(self, x):
        return F.conv2d(
            ...
            torch.nn.Parameter(
                self.weight
                + torch.rot90(self.weight, 1, [-1, -2])
                + torch.rot90(self.weight, 2, [-1, -2])
                + torch.rot90(self.weight, 3, [-1, -2]) / 4.0
            ),
            torch.nn.Parameter(self.bias),
            1,
            self.kernel_size - 1,
        )
...
class SynthesisLayer(torch.nn.Module):
    def __init__(
        ...
        self.tmp_sampling_rate = (
            max(in_sampling_rate, out_sampling_rate) * lrelu_upsampling
        )
        ...
        self.up_taps = (
            filter_size * self.up_factor if self.up_factor > 1 else 1
        )
        ...
        self.down_taps = (
            filter_size * self.down_factor if self.down_factor > 1 else 1
        )
        self.down_radial = (
    ...
    def forward(
        self, x, noise_mode='random', force_fp32=False, update_emas=False
    ):
        dtype = torch.float32
    ...
    def forward(self, x):
        if self.in_sampling_rate == 2 * self.out_sampling_rate:
            ...
            return nn.functional.interpolate(
                self.activation(x), size=self.out_size
            )
...
class LReLu_torch(torch.nn.Module):
    ...
    def forward(self, x):
        x = nn.functional.interpolate(
            x, size=2 * self.in_size, mode='bicubic', antialias=True
        )
        x = self.activation(x)
        x = nn.functional.interpolate(
            x, size=self.in_size, mode='bicubic', antialias=True
        )
        x = nn.functional.interpolate(
            x, size=self.out_size, mode='bicubic', antialias=True
        )
