...
def _conv2d_wrapper(
    ...
    op = (
        conv2d_gradfix.conv_transpose2d if transpose else conv2d_gradfix.conv2d
    )
...
def conv2d_resample(
    ...
    assert (
        isinstance(w, torch.Tensor) and (w.ndim == 4) and (w.dtype == x.dtype)
    )
    assert f is None or (
    ...
    if up > 1:
        if groups == 1:
            ...
            w = w.reshape(
                groups, out_channels // groups, in_channels_per_group, kh, kw
            )
            w = w.transpose(1, 2)
            w = w.reshape(
